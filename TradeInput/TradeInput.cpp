...
void TradeGenerator()
{
	std::fstream file("trades.txt", std::ios::out | std::ios::trunc); // open the file
	...
	if (file.is_open())
	{
		...
		for (int i = 0; i <10* 6; i++)
		{
			...
			double tradeprice = 99.0 + ((temp < 512) ? temp / 256.0 : (1024 - temp) / 256.0);
			string stringprice = MakePrice(tradeprice);
			string side;
			if (i % 2 == 0)side = string("BUY");
			if (i % 2 != 0) side = string("SELL");
			string quantity = to_string((i % 5 + 1) * 1000000);
			string tradeid = CreateId();
			string book;
			if (i % 3 == 0)book = string("TRSY1");
			if (i % 3 == 1) book = string("TRSY2");
			if (i % 3 == 2)book = string("TRSY3");
		
			// make the output
			file << bondcusip << "," << tradeid << "," << stringprice <<"," <<book <<","<<quantity<<","<<side<< "\n";
		}
