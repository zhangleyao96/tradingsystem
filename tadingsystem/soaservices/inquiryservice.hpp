...
public:
  ...
  void ChangeState(string _state) {
	  if (_state == "RECEIVED") state = RECEIVED;
	  if (_state == "QUOTED") state = QUOTED;
	  if (_state == "DONE") state = DONE;
	  if (_state == "REJECTED") state = REJECTED;
	  if (_state == "CUSTOMER_REJECTED") state = CUSTOMER_REJECTED;
  }
